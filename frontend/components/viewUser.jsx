...
import { Button, Container, Input, Stack, Text, HStack, Table, Thead, Tbody, Tfoot, Tr, Th, Td, Heading, CloseButton, Icon} from "@chakra-ui/react";
...
const viewUser = () => {
  ...
  const deletUser = async ( id ) =>{
    const response = await axios.delete(`${process.env.API_URL}/deleteUser/`+id)
    getUsers();
  }
        ...
        return (
          ...
          >
            ...
            <Td textAlign='center' display={'flex'} justifyContent='space-around'>
              <Button as={BsTrashFill} boxSize={'35'} onClick={() => deletUserConfirmation(usuario._id)}></Button>
              <Button as={VscAdd} boxSize={`35`} onClick={() => router.push(`/usuario/${usuario._id}`)}> </Button>
            </Td>
          </Tr>
        )
    }) 
  ...
  return (
    <>
        ...
        <Heading textAlign="center" my={10} >LISTA DE USUARIOS</Heading>
        <Button colorScheme={'teal'} size='lg' onClick={()=>router.push('/CreateUser')}>Crear Usuario</Button>
            <Table variant={"simple"} my='10'>
                <Thead color='white'>
                    <Tr borderColor={'black'} bg={'black'} borderWidth='2px' textTransform={'uppercase'}>
